...
setwd("C:/Users/chise/Desktop/screen-analysis-for-yellow-Tmaze-and-Joystick/data/Joystick")


########################################## Initialize some parameters  ###########################################


tested_flies <- read.table("./positive_control.txt", quote="\"", comment.char="")

skip<-37
flat_thres <- 0.6
PI_thres <- 2
cross_thres <- 2
plot_title <- "TrpA1+Gr28bd>Chrimson n=12"
...
# Plot of the PIs
plot(mean_trace, type = "l",xlab = "data points",ylab = "Position", main = plot_title)
abline(v = segment, untf = FALSE, col="red",lwd=3)

#plot(sd_trace, type = "l",xlab = "data points",ylab = "Standard deviation", main = "TH-D'>Chrimson n=11")
#abline(v = segment, untf = FALSE, col="red",lwd=3)
boxplot(SD_platform, col="grey",xlab="",ylab="SD",main= plot_title, ylim = c(0, 3.5),names=c("Pretest","Pretest","Training","Training","Test","Test","Training","Training","Test","Test"), cex.lab=1.5, cex.axis = 1.2)
group <- NULL
for(i in 1:10){
  a <- rep(i,Nexp*3)
