...
setwd("C:/Users/hp/Desktop/new_screens/all")


########################################## Initialize some parameters  ###########################################


tested_flies <- read.table("C:/Users/hp/Desktop/new_screens/all/all.txt", quote="\"", comment.char="")
...
Nexp_total <- length(tested_flies$V1)


effectsize <- vector("numeric", length = 3*Nexp_total) # Saurabh variable defining effectsize score
effectsize_mat <- matrix(NA, 3*Nexp_total, no_of_screens)

on_wiggle <- vector("numeric", length = 3*Nexp_total)
off_wiggle <- vector("numeric", length = 3*Nexp_total)
  ...
  
{
  ...
  on_wiggle <- vector("numeric", length = 3*Nexp)
  off_wiggle <- vector("numeric", length = 3*Nexp)
  diff_wiggle <- vector("numeric", length = 3*Nexp)
  
  effectsize <- vector("numeric", length = 3*Nexp) # Saurabh variable defining effectsize score
  ...
  for(i in 1:Nexp){
    ...
    info <-read.table(as.character(tested_flies$V1[i+x]), header = FALSE, sep = "", 
    ...
    for(oo in 1:5){
      combo_PI_platform1[oo] <- calculate_PI(data$state1[sau_segment[oo]:sau_segment[oo+1]])
      combo_PI_platform2[oo] <- calculate_PI(data$state2[sau_segment[oo]:sau_segment[oo+1]])
      combo_PI_platform3[oo] <- calculate_PI(data$state3[sau_segment[oo]:sau_segment[oo+1]])
    }
    
    effectsize1 <- (combo_PI_platform1[2]+combo_PI_platform1[4])/2-combo_PI_platform1[1]
    effectsize2 <- (combo_PI_platform2[2]+combo_PI_platform2[4])/2-combo_PI_platform2[1]
    effectsize3 <- (combo_PI_platform3[2]+combo_PI_platform3[4])/2-combo_PI_platform3[1]
  ...
  effectsize_mat <- effectsize
  on_wiggle_mat <- on_wiggle
  off_wiggle_mat <- off_wiggle
  ...
  matplot(t(PI_platform[accepted_flies,]), type = c("b"),pch=1,col = 1:4)
...
barplot(mean_effectsize,col="yellow",xlab="",ylab="reinforcement", cex.lab=1.0, cex.axis = 0.5)
abline(h = 0, untf = FALSE, col="black",lwd=3)
